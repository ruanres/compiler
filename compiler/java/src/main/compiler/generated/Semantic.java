...
public class Semantic {
	...
	public void checkAssignVariableIsValid(Type variableType, Variable var) {
		...
		//Inicialmente a variavel È associada com o o mesmo tipo da express„o que foi dado assign. SÛ aqui È verificado se os tipos batem		
		if (!variableType.equals(expressionAssignedType)) {
			throw new SemanticException("N„o È possivel a variavel do tipo " + variableType.getName() + " ser associado com um valor/variavel do tipo " +  expressionAssignedType);
		}
	...
	public void checkAssignVariableIsValid(Variable var, Expression exp) {
		...
		if (!currentScopeVariable.getType().equals(exp.getType())) {
			throw new SemanticException("N„o È possivel a variavel do tipo " + currentScopeVariable.getName() + " ser associado com um valor/variavel do tipo " +  exp.getType());
		}
	...
	public boolean isRelationalExpression(Expression le, Expression re) throws SemanticException {
		if(!le.getType().equals(re.getType())){
            throw new SemanticException("ERRO: A express√£o formada pelas subexpress√µes de valor " + le.getValue() + " do tipo "
                    + le.getType().getName()+" e de valor " + re.getValue() + " do tipo " + re.getType().getName()+ " n√£o √© uma express√£o relacional!");
        }
