...
public class Semantic {
    ...
    public void checkIfExpression(Expression e) throws SemanticException {
        boolean isIntType = e.getType().toString().equals("int");
        if (!isIntType) {
            throw new SemanticException("The IF expression is not of type int.");
        }
    }
    ...
    public Expression getExpressionForOperation(Operation op, Expression e1, Expression e2) {
        ...
        if (!branchCommand.isEmpty()) {
            getCodeGenerator().generateSUBCode();
            getCodeGenerator().enqueuBranchCommmand(branchCommand);
        }
    ...
    public void checkSelectionStatement(Expression e) {
        checkIfExpression(e);
        getCodeGenerator().generateIFTHENCode(e);
    }
